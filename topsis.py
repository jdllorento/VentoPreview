...
# 8. Definición de la función TOPSIS
def topsis(X_norm, pesos, tipos):
    V = X_norm * pesos
    ...
    S_pos = np.sqrt(((V - pd.Series(ideal_pos))**2).sum(axis=1))
    S_neg = np.sqrt(((V - pd.Series(ideal_neg))**2).sum(axis=1))

    C = S_neg / (S_pos + S_neg)

    return C.sort_values(ascending=False), V, ideal_pos, ideal_neg


# 9. RANKING OUTPUT

ranking_topsis, V_ponderada, ideal_pos, ideal_neg = topsis(X_norm, pesos_sel, tipos_sel)
